...
![1617718920488](C:\Users\Amd\AppData\Roaming\Typora\typora-user-images\1617718920488.png)



## 事件轮询

浏览器是一个进程，js是浏览器的一个线程（单线程）。
...
+ 浏览器管理模块（I/O）
  ...
  检测到模块相关代码或操作时，对代码或操作进行处理，将其有序放入回调队列

+ 回调队列

  

+ event-loop 挨个执行回调队列中的任务

  监听执行栈和回调队列

  执行栈为空时，将回调队列的数据有序的放入到执行栈中进行执行
...
2 - js执行时遇到DOM事件、ajax事件、定时器等**异步功能**时，放入对应的模块，对应模块处理后，将其放入回调队列中

3 - 同步代码执行完毕时，event-loop从回调队列开始挨个执行异步任务
```
